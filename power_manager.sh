...
# and updates the status of a virtual (dummy) NUT server. This allows NUT
# clients on the network to perform a graceful shutdown. It also handles
# power restoration by sending Wake-on-LAN packets to designated servers.
#
################################################################################
...
# Loop through each host and check its status via exit code
for IP in $SENTINEL_HOSTS; do
    $PING_CMD -c 1 -W 1 "$IP" &> /dev/null
    if [ $? -eq 0 ]; then
